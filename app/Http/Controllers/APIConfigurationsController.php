...
class APIConfigurationsController extends Controller
{
	...
	public function create_update_class_manual(Request $request)
	{
        ...
        $userall_id = UserAll::where(['usesr_table_id'=>$user_details->id,'user_role'=>$user_data->user_role])->pluck('id')->first();//fetch id from user all table to store notification triggered user
        $status = 'insert';
    ...
    public function onboarding_fetch_single_parent(Request $request)
    {
		...
		$student_list = ([
			...
			'father_image'=>isset($parentsdata[1])?$parentsdata[1]->profile_image:null,
			...
			'mother_image'=>isset($parentsdata[2])?$parentsdata[2]->profile_image:null,
	...
	public function onboarding_create_students(Request $request)
	{
        ...
        foreach ($data as $row) {
        	$student_image = $group_id = $password = $gender='';
	...
	public function configuration_list()
	{
        ...
        $profile = SchoolDatabase::where(['school_id'=>$userdata->school_profile_id])->get()->first();
       	$excel =(file_exists(public_path('/uploads/Onboarding.xlsx'))?Config::get('app.QA_URL').'uploads/Onboarding.xlsx':'');
        // return token 
	...
	public function all_management_list(Request $request)
	{
        ...
        foreach ($list as $key => $value) {
        	$schoolcheck_access = SchoolUsers::select('id','new_user','user_status')->where('user_id',$value['user_id'])->where('user_role',Config::get('app.Management_role'))->first(); //2- full deactivate
        	if($schoolcheck_access->user_status == 1)
        	{
        		$check_group_access = UserGroupsMapping::where('user_table_id',$value['id'])->where('user_role',Config::get('app.Management_role'))->pluck('id')->first();
        	...
        	$member_management_list['data'][]=([
	            ...
	            'doj' => $value['doj'],
	            'employee_no' => $value['employee_no'],
	            'user_status' => ($check_access == null)?3:$value['user_status'], // 1- active,2-full deactive,,3-partical deactive
