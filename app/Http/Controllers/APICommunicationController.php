...
class APICommunicationController extends Controller
{
    ...
    public function view_messages(Request $request)
    {
        ...
        if(!empty($userdetails) && !empty($group_id))
        {
            ...
            $total_notification_ids = CommunicationRecipients::where(['user_table_id'=>$userdetails->id,'user_role'=>$user->user_role])->orderBy('actioned_time','DESC')->get()->toArray();

            $notification_ids = array_filter($total_notification_ids, function ($item) use ($communication_id_list,$newsevents_id_list) {
                if((($item['communication_type'] == 1 || $item['communication_type'] == 4) && in_array($item['communication_id'],$communication_id_list)) || ($item['communication_type'] == 2 && in_array($item['communication_id'],$newsevents_id_list))) 
                    return $item;
            });
            // Chat message ids
    ...
    public function message_delivery_details(Request $request)
    {
        ...
        if(!empty($message_details))
        {
            ...
            if(!empty($delivery_details))
            {
                $delivered_users=[];
                $index=0;
                $all_categories_list = UserCategories::select('id','user_category')->first();
            ...
            $count = count(array_filter($approval_data, function ($item) {
                if(($item['communication_type'] == 2 && (date('Y-m-d',strtotime($item['actioned_time'])) >= date("Y-m-d",strtotime(Carbon::now()->timezone('Asia/Kolkata')))) ) || $item['communication_type'] == 1)
                {
                    if($item['communication_type'] == 2)
                        ...
                        $fetch_sender_id = CommunicationRecipients::select('user_table_id','user_role')->where(['view_type'=>1,'communication_id'=>$item['id']])->first();
                    if(!empty($fetch_sender_id))
                        return $item;
                }
            }));
