...
class APICommunicationController extends Controller
{
    ...
    public function view_messages(Request $request)
    {
        ...
        if(!empty($userdetails) && !empty($group_id))
        {
            ...
            if($visible_to!='')
                $class_messages = Communications::where('group_id',2)->where('visible_to','like','%,'.$visible_to.',%')->where(['distribution_type'=>6,'communication_type'=>1])->orwhere(['distribution_type'=>8,'communication_type'=>1])->pluck('id')->toArray();
