...
class APIAttendanceController extends Controller
{
    ...
    public function get_attendance(Request $request)
    {
        ...
        foreach($student_details as $key=>$value)
        {
            ...
            $student_details[$key]['absent_total'] = $absent_total;
            $student_details[$key]['leave_total'] = $leave_total;
            $student_details[$key]['present_percentage'] = ($leave_total > 0)?(($leave_total/$total_days)*100):0;
            $student_details[$key]['absent_percentage'] = ($absent_total > 0)?(($absent_total/$total_days)*100):0;
            $student_details[$key]['leave_percentage'] = ($leave_total > 0)?(($leave_total/$total_days)*100):0;
