...
class APIConfigurationsController extends Controller
{
   ...
   // 			File::copy(storage_path('app/Staffs.xlsx'), public_path(env('SAMPLE_CONFIG_URL').$school_profile->school_code.'/'.'Staffs.xlsx'));
	...
	public function user_group_list()
	{
        ...
        $check_access = UserGroupsMapping::where('user_table_id',$userid)->where('group_type',2)->where('user_role',$user_data->user_role)->where('user_status',1)->pluck('id')->first();
    ...
    public function users_count()
    {
		...
		$deactiveusercount = SchoolUsers::select(DB::raw('count(*) as count'))->where('school_profile_id',$user->school_profile_id)->where('user_status',2)->get()->first();
