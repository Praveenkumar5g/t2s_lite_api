...
class APICommunicationController extends Controller
{
    ...
    public function view_messages(Request $request)
    {
        ...
        if(!empty($userdetails) && !empty($group_id))
        {
            ...
            $communication_id_list = array_unique(array_merge($chat_id_list,$remaining_id_list,$class_messages,$student_messages));
            ...
            $notification_ids = CommunicationRecipients::where(function($query) use ($communication_id_list,$userdetails,$user) {
                    $query->where(['user_tablex_id'=>$userdetails->id,'user_role'=>$user->user_role,'communication_type'=>1])->whereIn('communication_id',$communication_id_list);
